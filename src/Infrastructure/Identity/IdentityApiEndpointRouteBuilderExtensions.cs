...
public static class IdentityApiEndpointRouteBuilderExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
            ([FromBody] AccountRegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            if (!userManager.SupportsUserEmail)
            {
                throw new NotSupportedException(
                    $"{nameof(MyMapIdentityApi)} requires a user store with email support.");
            }
                   ...
                   await dbContext.Database.BeginTransactionAsync(CancellationToken.None))
            {
                try
                {
                    ...
                    var resultRegister = await accountService.RegisterAsync(registration, Roles.Manager);
                    if (!resultRegister.Succeeded)
                    {
                        await transaction.RollbackAsync(CancellationToken.None);
                        return TypedResults.ValidationProblem(
                            new Dictionary<string, string[]> { { "Errors", resultRegister.Errors } });
                    }
            ...
            [FromServices] IServiceProvider sp) =>
        {
            ...
            // 驗證帳戶狀態
            var dbContext = sp.GetRequiredService<IApplicationDbContext>();
            var account = await dbContext.Accounts.FirstOrDefaultAsync(a => a.Email == login.Email);
                
            if (account == null)
            {
                return TypedResults.Problem("Account not found", statusCode: StatusCodes.Status401Unauthorized);
            }
            
            if (account.Status == false)
            {
                return TypedResults.Problem("Account is disabled", statusCode: StatusCodes.Status401Unauthorized);
            }
            ...
            string email, bool isChange = false)
        {
            ...
            // If use HTML, then the email will be sent as HTML
            //await emailSender.SendConfirmationLinkAsync(user, email, HtmlEncoder.Default.Encode(confirmEmailUrl));
            
            // If use text, then the email will be sent as text
